            ...
            <tbody>
                @forelse($orders as $o)
                    <tr>
                        ...
                        <td>
                            {{ $o->ngay_dat 
                                ? \Carbon\Carbon::parse($o->ngay_dat)->format('d/m/Y H:i') 
                                : '-' }}
                        </td>
                        ...
                        <td>
                            <span class="badge 
                                {{ $o->trang_thai_tt == 'DA_THANH_TOAN' ? 'bg-success' : 'bg-warning' }}">
                                {{ $paymentLabels[$o->trang_thai_tt] ?? $o->trang_thai_tt }}
                            </span>
                        </td>

                        {{-- ✅ Trạng thái đơn hàng (label từ model + màu từ tabs) --}}
                        @php
                            $tab = $statusTabs[$o->trang_thai] ?? ['color' => 'secondary'];
                        @endphp
                        <td>
                        ...
                        <td>
                            <div class="d-flex gap-2">
                                <a href="{{ route('client.orders.show',$o->id_don_hang) }}"
                                ...
                                @if($o->trang_thai=='CHO_XAC_NHAN')
                                    <form method="POST"
                                          action="{{ route('client.orders.cancel',$o->id_don_hang) }}">
                                        @csrf
                                        @method('PUT')
